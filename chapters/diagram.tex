...
\def\nouncontent{%
    \begin{tikzpicture}[every node/.style={tight, diagram}]
        ...
        \node (pill1) [below=of rect1] {\pill[color=gray, width=2em, radius=0.8em, label=\hyperref[table:artdef]{art.def.}]};%
        \node (pill2) [right=of pill1] {\pill[color=gray, width=2em, radius=0.8em, label={pron.3.per}]};%
        \node (pill3) [right=of pill2] {\pill[color=gray, width=2em, radius=0.8em, label={adj.str.}]};%
        \draw[-, shorten >=0, shorten <=0] (pill1) -- (pill2) node[midway, above] {};%
        \draw[-, shorten >=0, shorten <=0] (pill2) -- (pill3) node[midway, above] {};%

        \node (pill4) [below=of pill1] {\pill[color=gray, width=2em, radius=0.8em, label={art.indef.}]};%
        \node (pill5) [right=of pill4] {\pill[color=gray, width=2em, radius=0.8em, label={pron.3.pos.}]};%
        \node (pill6) [right=of pill5] {\pill[color=gray, width=2em, radius=0.8em, label={adj.wk.}]};%
...
\begin{tikzpicture}[]
    ...
    \draw[-latex, shorten >=0, shorten <=0] (card2) -- (card1) node[midway, right] {case};
    \draw[-latex, shorten >=0, shorten <=0] (card3) -- (card1) node[midway, left] {case};
\end{tikzpicture}