...
\renewcommand{\arraystretch}{1.5}{
    \begin{tabular}{|M{2.5em}|M{3em}|M{2.5em}|M{3em}|}
        ...
        格 & 形                    & 性 & 色                     \\
        \hline
        N & \square[border=true] & M & \square[color=green]  \\
        \hline
        A & \diam[border=true]   & N & \square[color=purple] \\
        \hline
        D & \olive[border=true]  & F & \square[color=yellow] \\
        \hline
        G & \tri[border=true]    & P & \square[color=red]    \\
        \hline
    \end{tabular}
}
...
\section{名词性、数规律}
使用方法：由上到下看图。先看词形，若匹配后缀，可确定（彩色的）或者猜出（灰色的）对应的性（绿，紫，黄）、数（红，有5个能直接确定）。若无匹配，再看所属词族，若匹配，则可猜出性。根据性和音节数，便可以猜出复数形式。总之：\\
\begin{myquote}
    形义推性，性音推数，否则特殊。
\end{myquote}
\tikzset{
...
\begin{tikzpicture}[glossary]
    ...
    \node(f1) [feminine, right=0.6cm of p3] {-ei, -heit, -keit,\\-ung, -schaft};
    ...
    \node(pp3) [draw=red, text=red, fit=(tt1) (tt2)] {\textcolor{gray}{polysylabic:}-e};

    \node(tt3) [draw=none] at ($(r4.center)!(pp1.center)!(r4.center)$) {};
    \node(tt4) [draw=none, below=0.3cm of tt3] {};
    \node(pp4) [draw=red, text=red, fit=(tt3) (tt4)] {-n\textcolor{gray}{ (weak nouns)}};
